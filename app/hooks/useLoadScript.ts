  ...
  const promise = new Promise<boolean>((resolve, reject) => {
    ...
    script.onload = (e): void => {
      ...
      if (typeof onload === "function") {
        onload.call(script, e);
      }
    };
    script.onerror = (e): void => {
      reject(false);
      if (typeof onerror === "function") {
        onerror.call(script, e);
      }
    };
    if (placement === "head") {
...
export function useLoadScript(
  ...
  reload = false, // boolean whether to remove & re-add script on function call
): ScriptState {
  ...
  useEffect(() => {
    ...
    async function loadScriptWrapper(): Promise<void> {
      try {
        setStatus("loading");
        await loadScript(attributes, placement, reload);
        setStatus("done");
      } catch (error) {
        setStatus("error");
      }
