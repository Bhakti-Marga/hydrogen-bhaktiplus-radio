...
export async function loader({ request }: LoaderFunctionArgs) {
  // Return 404 status but don't throw - let the component render
...
export default function NotFoundPage() {
  ...
  return (
    <div className="not-found-page min-h-screen bg-brand-dark flex flex-col">
      {/* Header with logo and gold accent line */}
      <header className="not-found-header py-16 px-24 relative flex items-center justify-center">
        <Link to="/">
          <img
            src={imageLogo}
      ...
      {/* Main Content */}
      <main className="not-found-main flex-1 flex flex-col items-center justify-center px-24 pb-24">
        {/* Lotus Icon */}
        <div className="not-found-lotus mb-40">
          <LotusIcon className="w-[72px] h-[72px]" altText={strings.alt_bhakti_plus_lotus} />
        </div>

        {/* 404 Number */}
        <p className="text-64 md:text-80 font-600 text-white/20 font-figtree mb-8">
          {strings.error_404_code}
        </p>

        {/* Heading */}
        <h1 className="text-22 md:text-28 font-600 text-white text-center mb-16 font-figtree leading-tight">
          {strings.error_404_title}
        </h1>

        {/* Subtext */}
        <p className="text-16 text-white/60 text-center mb-48 max-w-[320px]">
          {strings.error_404_description}
        </p>

        {/* Action Button */}
        <Link
          to="/"
          className="block w-full max-w-[280px] py-16 px-32 text-center text-16 font-600 rounded-full bg-white text-brand transition-all duration-200 hover:opacity-90 hover:scale-[1.02]"
        >
          {strings.error_404_button}
        </Link>
      </main>

      {/* Footer */}
      <footer className="not-found-footer py-20 px-24">
        <nav className="not-found-footer-nav flex justify-center gap-20 mb-8">
          <a
 ...
 * White lotus icon
 */
function LotusIcon({ className, altText }: { className?: string; altText?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
