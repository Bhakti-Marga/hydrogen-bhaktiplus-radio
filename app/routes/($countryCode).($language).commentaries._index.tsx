...
async function loadCriticalData({ context }: LoaderFunctionArgs) {
  ...
  // Fetch commentaries and featured live in parallel
  const [{ commentaries }, featuredLiveResponse] = await Promise.all([
    userScopedMediaApi.commentaries.getList(),
    userScopedMediaApi.lives.getFeatured().catch(() => ({ featured: null })),
  ]);
  const featured =
    commentaries.find((commentary) => commentary.contentId === 20) ||
    commentaries[0];

  // If the featured live's ppvTag matches the featured commentary's ppvTag,
...
export default function CommentariesHomepage() {
  const { featured, activeLive, commentaries, subscriptionTier, user } =
            ...
            >
              <Await resolve={commentaries}>
                {(resolvedCommentaries) => {
                  // Filter to only commentaries user has access to
                  const myCommentaries = user
                    ? resolvedCommentaries.filter((commentary) =>
                        hasAccessToContent(user, subscriptionTier, commentary),
                      )
                    : [];

                  const hasAccessibleContent = myCommentaries.length > 0;

                  return (
                    <Stack gap={7}>
                      {hasAccessibleContent && (
                        <ContentSeriesCarousel
                          title={strings.my_commentaries || "My Commentaries"}
                          items={myCommentaries}
                          contentType="commentary"
                          exploreAllLink="/commentaries"
                        />
                      )}
                      <ContentSeriesCarousel
                        title={strings.all_commentaries || "All Commentaries"}
